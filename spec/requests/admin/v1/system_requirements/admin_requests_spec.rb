...
RSpec.describe 'Admin::V1::SystemRequirements as :admin', type: :request do
  ...
  context 'PATCH /system_requirements/:id' do
    ...
    context 'with valid params' do
      let(:system_requirement_name) { 'My new system_requirement' }
      let(:system_requirement_params) do
        { system_requirement: { name: system_requirement_name } }.to_json
      end
      ...
      it 'updates system_requirement' do
        ...
        expect(system_requirement.name).to eq system_requirement_name
      end
